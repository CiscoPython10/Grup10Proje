...
class Pencere(QtWidgets.QWidget):
    ...
    def tanima(self):
        ...
        recognizer.read('yuz_tan_vlk_deneme/training/trainer.yml')
        cascadePath = "yuz_tan_vlk_deneme/face.xml"
        faceCascade = cv2.CascadeClassifier(cascadePath)
        path = 'yuz_tan_vlk_deneme/yuzverileri'
    ...
    def egitme(self):
        ...
        if not self.timer.isActive():
            ...
            recognizer.write('yuz_tan_vlk_deneme/training/trainer.yml')
            cv2.destroyAllWindows()
        else:
            self.timer.stop()
            cv2.destroyAllWindows()
    ...
    def veriseti(self):
        ...
        if not self.timer.isActive():
            ...
            self.kamera=cv2.VideoCapture(0)
            self.detector=cv2.CascadeClassifier('yuz_tan_vlk_deneme/face.xml')
            ...
            while True:
                ...
                for(x,y,w,h) in faces:
                    i=i+1
                    cv2.imwrite("yuz_tan_vlk_deneme/yuzverileri/face-" + kisi_id + '.' + str(i) + ".jpg", gray[y:y + h , x :x + w])
